...
export default function EnneagramTest() {
  ...
  const currentQuestions = enneagramTestQuestions.slice(
    currentPage * questionsPerPage, 
    (currentPage + 1) * questionsPerPage
  );

  const progress = (Object.keys(answers).length / enneagramTestQuestions.length) * 100;
  
  const isPageComplete = currentQuestions.every(q => answers[q.id] !== undefined);

  const handleAnswer = (questionId, value) => {
    // value is the score directly mapped to strongly agree / strongly disagree
    // to map to enneagram logic (1 multiplier), we just pass the raw value
    setAnswers(prev => {
        const question = enneagramTestQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
            [questionId]: { value, type: question.type, multiplier: question.multiplier }
        };
    });
  };

  const handleNextPage = () => {
    if (currentPage < Math.ceil(enneagramTestQuestions.length / questionsPerPage) - 1) {
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
        // calculateEnneagramResult expects an array of answer objects
        const answersArray = Object.values(answers);
        const calculatedResult = calculateEnneagramResult(answersArray);
  ...
  if (result) {
    return <Navigate to={`/result/enneagram/${result.fullTitle}`} state={{ resultData: result }} replace />;
  }
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(enneagramTestQuestions.length / questionsPerPage)}</span>
          </div>
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(enneagramTestQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
    ...
    <div className={`flex flex-col items-center py-12 md:py-16 border-slate-100 w-full relative ${!isLast ? 'border-b' : ''}`}>
      ...
      <div className="hidden sm:flex items-center justify-center w-full max-w-4xl mt-4">
        <div className="flex-1 flex justify-end pr-4 md:pr-8">
          <span className="text-[#10b981] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-right leading-tight">Highly Accurate</span>
        </div>
        ...
        <div className="flex-1 flex justify-start pl-4 md:pl-8">
          <span className="text-[#6366f1] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-left leading-tight">Highly Inaccurate</span>
        </div>
      </div>
      ...
      <div className="flex sm:hidden flex-col items-center w-full px-1 mt-6">
        ...
        <div className="flex justify-between w-full px-2 text-[0.65rem] uppercase font-semibold tracking-widest mt-6 opacity-60">
          <span className="text-[#10b981]">Highly Accurate</span>
          <span className="text-[#6366f1]">Highly Inaccurate</span>
        </div>
      </div>
    </div>
  );
