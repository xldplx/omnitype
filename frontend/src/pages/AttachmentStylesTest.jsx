...
export default function AttachmentStylesTest() {
  ...
  const currentQuestions = attachmentTestQuestions.slice(
    currentPage * questionsPerPage, 
    (currentPage + 1) * questionsPerPage
  );

  const progress = (Object.keys(answers).length / attachmentTestQuestions.length) * 100;
  
  const isPageComplete = currentQuestions.every(q => answers[q.id] !== undefined);

  const handleAnswer = (questionId, value) => {
    // value mapped to 1-5 scale
    setAnswers(prev => {
        const question = attachmentTestQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(attachmentTestQuestions.length / questionsPerPage) - 1) {
  ...
  const handleFinish = () => {
    ...
    const calculatedResult = calculateAttachmentResult(answersArray);
  ...
  // We use the ID (e.g., secure) so the URL is clean.
  if (result) {
    return <Navigate to={`/result/attachment-styles/${result.info.id}`} state={{ resultData: result }} replace />;
  }
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(attachmentTestQuestions.length / questionsPerPage)}</span>
          </div>
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(attachmentTestQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
    ...
    <div className={`flex flex-col items-center py-12 md:py-16 border-slate-100 w-full relative ${!isLast ? 'border-b' : ''}`}>
      ...
      <div className="hidden sm:flex items-center justify-center w-full max-w-4xl mt-4">
        <div className="flex-1 flex justify-end pr-4 md:pr-8">
          <span className="text-[#6366f1] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-right leading-tight">Not True For Me</span>
        </div>
        ...
        <div className="flex-1 flex justify-start pl-4 md:pl-8">
          <span className="text-[#10b981] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-left leading-tight">Very True For Me</span>
        </div>
      </div>
