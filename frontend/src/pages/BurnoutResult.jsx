...
import { ChevronLeft, Shield, Heart, Activity, Target, Flame, AlertTriangle, MessageSquare, Zap, ShieldAlert, Moon, Battery, Stethoscope } from 'lucide-react';
  ...
  if (!resultData && type) {
    ...
    if (burnoutStatuses[statusKey]) {
        resultData = {
           ...
           breakdown: { physicalPercent: 85, cognitivePercent: 90, nervousPercent: 95 } // Simulated
        };
    }
  }
      ...
      >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          <div className="lg:col-span-4 bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_40px_rgb(0,0,0,0.06)] rounded-[2.5rem] p-10 flex flex-col items-center justify-center relative overflow-hidden group text-center hover:-translate-y-2 transition-transform duration-500">
             ...
             <span className="text-xs md:text-sm font-bold tracking-widest uppercase text-slate-600 mt-6 z-10 whitespace-nowrap bg-white/50 px-4 py-2 rounded-full border border-white/80">Current State</span>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_40px_rgb(0,0,0,0.06)] rounded-[2.5rem] p-10 md:p-16 mb-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 border-b border-slate-100 pb-8 gap-4">
            <div className="flex items-center gap-3">
                ...
                <h4 className="text-sm font-bold tracking-[0.3em] text-slate-500 uppercase">Exhaustion Metrics</h4>
            </div>
            <div className="flex items-center gap-2 bg-slate-100 px-5 py-2.5 rounded-full border border-slate-200">
                <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />
                <span className="text-xs font-black text-slate-600 uppercase tracking-widest">System Load</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 sm:gap-16">
              <MetricGauge label="Physical Exhaustion" value={breakdown.physicalPercent} color={info.color} icon={Target} delay={0.2} />
              <MetricGauge label="Cognitive Fatigue" value={breakdown.cognitivePercent} color={info.color} icon={Flame} delay={0.4} />
              <MetricGauge label="Nervous System Stress" value={breakdown.nervousPercent} color={info.color} icon={Zap} delay={0.6} />
          </div>
        </motion.div>

        {/* NEW: System Recovery Protocol (Dark Mode Section) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <span className="text-[0.65rem] font-bold tracking-widest text-white/50 uppercase block mb-1">Prescribed Action</span>
                            <h5 className="text-2xl font-bold text-white tracking-tight leading-none">{info.recoveryPath}</h5>
                        </div>
                    </div>
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <span className="text-[0.65rem] font-bold tracking-widest text-white/50 uppercase block mb-1">Primary Energy Drain</span>
                            <h5 className="text-2xl font-bold text-white tracking-tight leading-none">The Root Cause</h5>
                        </div>
                    </div>
                    <p className="text-slate-400 text-lg leading-relaxed font-medium pl-16">
                        {info.energyDrain}
                ...
                {/* Warning Signs & Rest Type */}
                <div className="flex flex-col md:col-span-2 mt-4 bg-white/5 p-8 rounded-3xl border border-white/10">
                    <div className="flex items-center gap-4 mb-4">
                        <ShieldAlert className="w-6 h-6 text-rose-400" />
                        <h5 className="text-xl font-bold text-white tracking-tight">Early Warning Signs & Rest Requirements</h5>
                    </div>
                    <div className="pl-10 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <span className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2 flex items-center gap-2"><AlertTriangle className="w-3 h-3 text-rose-400" /> Watch Out For</span>
                            <p className="text-slate-300 text-[1.05rem] leading-relaxed">{info.warningSigns}</p>
                        </div>
                        <div>
                            <span className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2 flex items-center gap-2"><Moon className="w-3 h-3 text-indigo-400" /> Required Rest Type</span>
                            <p className="text-slate-300 text-[1.05rem] leading-relaxed font-bold">{info.restType}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2.5rem] p-10 md:p-16 mb-8 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6">
                  <h5 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2"><Target className="w-4 h-4 text-slate-400"/> Physical State</h5>
                  <p className="text-slate-600 text-[1.05rem] leading-relaxed">{info.traits?.physical}</p>
               </div>
               <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6">
                  <h5 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2"><Flame className="w-4 h-4 text-slate-400"/> Cognitive State</h5>
                  <p className="text-slate-600 text-[1.05rem] leading-relaxed">{info.traits?.cognitive}</p>
               </div>
               <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6">
                  <h5 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2"><Zap className="w-4 h-4 text-slate-400"/> Nervous System</h5>
                  <p className="text-slate-600 text-[1.05rem] leading-relaxed">{info.traits?.nervous}</p>
               </div>
            </div>
        </motion.div>
