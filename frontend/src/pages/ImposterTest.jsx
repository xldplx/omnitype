...
export default function ImposterTest() {
  ...
  const currentQuestions = imposterTestQuestions.slice(
    currentPage * questionsPerPage, 
    (currentPage + 1) * questionsPerPage
  );

  const progress = (Object.keys(answers).length / imposterTestQuestions.length) * 100;
  
  const isPageComplete = currentQuestions.every(q => answers[q.id] !== undefined);

  const handleAnswer = (questionId, value) => {
    setAnswers(prev => {
        const question = imposterTestQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(imposterTestQuestions.length / questionsPerPage) - 1) {
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
        ...
        const calculatedResult = calculateImposterResult(answersArray);
  ...
  if (isFinished && !result) {
    return (
      <div className="w-full min-h-screen bg-[#fafafa] flex flex-col items-center justify-center pt-24 pb-32">
        ...
        >
          <div className="w-12 h-12 border-4 border-slate-200 border-t-fuchsia-600 rounded-full animate-spin mb-8" />
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-slate-800 tracking-tight">Analyzing the Inner Critic</h2>
          <p className="text-slate-500 text-[1.1rem] tracking-wide font-medium flex items-center justify-center gap-2">
            Evaluating competency scripts<motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}>...</motion.span>
          </p>
        </motion.div>
      </div>
    );
  ...
  if (result) {
    return <Navigate to={`/result/imposter/${result.fullTitle}`} state={{ resultData: result }} replace />;
  }
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(imposterTestQuestions.length / questionsPerPage)}</span>
          </div>
          <div className="h-1.5 w-full bg-slate-100 overflow-hidden rounded-full">
            <motion.div 
              className="h-full bg-linear-to-r from-fuchsia-400 to-purple-500"
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(imposterTestQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
    ...
    <div className={`flex flex-col items-center py-12 md:py-16 border-slate-100 w-full relative ${!isLast ? 'border-b' : ''}`}>
      ...
      <div className="hidden sm:flex items-center justify-center w-full max-w-4xl mt-4">
        <div className="flex-1 flex justify-end pr-4 md:pr-8">
          <span className="text-[#10b981] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-right leading-tight">Like Me</span>
        </div>
        ...
        <div className="flex-1 flex justify-start pl-4 md:pl-8">
          <span className="text-[#6366f1] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-left leading-tight">Not Me</span>
        </div>
      </div>
      ...
      <div className="flex sm:hidden flex-col items-center w-full px-1 mt-6">
        ...
        <div className="flex justify-between w-full px-2 text-[0.65rem] uppercase font-semibold tracking-widest mt-6 opacity-60">
          <span className="text-[#10b981]">Like Me</span>
          <span className="text-[#6366f1]">Not Me</span>
        </div>
      </div>
    </div>
  );
