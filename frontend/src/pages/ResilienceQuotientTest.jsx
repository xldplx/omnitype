...
export default function ResilienceQuotientTest() {
  ...
  const totalPages = Math.ceil(rqQuestions.length / questionsPerPage);
  ...
  const currentQuestions = rqQuestions.slice(
    currentPage * questionsPerPage,
    (currentPage + 1) * questionsPerPage
  );

  const handleAnswer = (questionId, value) => {
    const questionData = rqQuestions.find(q => q.id === questionId);
    setAnswers(prev => ({ 
      ...prev, 
  ...
  const handleNextPage = () => {
    if (currentPage < totalPages - 1) {
      ...
      setTimeout(() => {
        ...
        const resultData = calculateRQ(answersArray);
        navigate(`/result/resilience/${resultData.rqInfo.id}`, { state: { resultData } });
  ...
  if (isAnalyzing) {
    return (
      <div className="w-full min-h-screen bg-[#fafafa] flex flex-col items-center justify-center pt-24 pb-32">
        ...
        >
          <div className="w-12 h-12 border-4 border-slate-200 border-t-sky-500 rounded-full animate-spin mb-8" />
  ...
  // ----------------------------------------
  const progress = (Object.keys(answers).length / rqQuestions.length) * 100;
    ...
    <div className="w-full min-h-screen bg-white pt-24 pb-32 relative overflow-hidden">
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          ...
          <div className="h-1.5 w-full bg-slate-100 overflow-hidden rounded-full mt-1">
            <motion.div 
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === totalPages - 1 ? 'Analyze Resilience' : 'Continue'}</span>
