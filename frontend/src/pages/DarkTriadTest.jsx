...
export default function DarkTriadTest() {
  ...
  const currentQuestions = darkTriadTestQuestions.slice(
    currentPage * questionsPerPage, 
    (currentPage + 1) * questionsPerPage
  );

  const progress = (Object.keys(answers).length / darkTriadTestQuestions.length) * 100;
  
  const isPageComplete = currentQuestions.every(q => answers[q.id] !== undefined);

  const handleAnswer = (questionId, value) => {
    setAnswers(prev => {
        const question = darkTriadTestQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(darkTriadTestQuestions.length / questionsPerPage) - 1) {
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
        ...
        const calculatedResult = calculateDarkTriadResult(answersArray);
  ...
  if (isFinished && !result) {
    return (
      <div className="w-full min-h-screen bg-[#fafafa] flex flex-col items-center justify-center pt-24 pb-32">
        ...
        >
          <div className="w-12 h-12 border-4 border-slate-200 border-t-zinc-800 rounded-full animate-spin mb-8" />
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-slate-800 tracking-tight">Compiling Shadow Profile</h2>
  ...
  if (result) {
    return <Navigate to={`/result/dark-triad/${result.fullTitle}`} state={{ resultData: result }} replace />;
  }
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(darkTriadTestQuestions.length / questionsPerPage)}</span>
          </div>
          <div className="h-1.5 w-full bg-slate-100 overflow-hidden rounded-full">
            <motion.div 
              className="h-full bg-linear-to-r from-zinc-600 to-zinc-900"
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(darkTriadTestQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
