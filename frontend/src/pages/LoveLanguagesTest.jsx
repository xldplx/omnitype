...
export default function LoveLanguagesTest() {
  ...
  const progress = (Object.keys(answers).length / loveLanguagesTestQuestions.length) * 100;
  ...
  const handleAnswer = (questionId, value) => {
    ...
    setAnswers(prev => {
        const question = loveLanguagesTestQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
  ...
  // We use the ID (e.g., words_of_affirmation) so the URL is clean.
  if (result) {
    return <Navigate to={`/result/love-languages/${result.info.id}`} state={{ resultData: result }} replace />;
  }
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(loveLanguagesTestQuestions.length / questionsPerPage)}</span>
          </div>
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(loveLanguagesTestQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
