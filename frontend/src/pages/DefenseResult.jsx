...
import { ChevronLeft, Shield, Heart, Activity, Target, Flame, AlertTriangle, MessageSquare, Zap, ShieldAlert, EyeOff, BrainCircuit } from 'lucide-react';
...
export default function DefenseResult() {
  ...
  if (!resultData && type) {
    ...
    if (defenseStatuses[statusKey]) {
        resultData = {
           ...
           breakdown: { logicPercent: 85, projectionPercent: 40, avoidancePercent: 30 } // Simulated
        };
    }
  }
  ...
  return (
    <div className="w-full min-h-screen bg-[#fafafa] relative overflow-hidden flex flex-col items-center selection:bg-sky-200 uppercase-none">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          <div className="lg:col-span-4 bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_40px_rgb(0,0,0,0.06)] rounded-[2.5rem] p-10 flex flex-col items-center justify-center relative overflow-hidden group text-center hover:-translate-y-2 transition-transform duration-500">
             ...
             <span className="text-xs md:text-sm font-bold tracking-widest uppercase text-slate-600 mt-6 z-10 whitespace-nowrap bg-white/50 px-4 py-2 rounded-full border border-white/80">Primary Defense</span>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_40px_rgb(0,0,0,0.06)] rounded-[2.5rem] p-10 md:p-16 mb-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 border-b border-slate-100 pb-8 gap-4">
            <div className="flex items-center gap-3">
                ...
                <h4 className="text-sm font-bold tracking-[0.3em] text-slate-500 uppercase">Defense Matrix</h4>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 sm:gap-16">
              <MetricGauge label="Intellectualization" value={breakdown.logicPercent} color={info.color} icon={Target} delay={0.2} />
              <MetricGauge label="Projection" value={breakdown.projectionPercent} color={info.color} icon={Flame} delay={0.4} />
              <MetricGauge label="Avoidance" value={breakdown.avoidancePercent} color={info.color} icon={EyeOff} delay={0.6} />
          </div>
        </motion.div>

        {/* NEW: The Anxiety Shield (Dark Mode Section) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <span className="text-[0.65rem] font-bold tracking-widest text-white/50 uppercase block mb-1">The Core Trigger</span>
                            <h5 className="text-2xl font-bold text-white tracking-tight leading-none">What Activates The Shield</h5>
                        </div>
                    </div>
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <span className="text-[0.65rem] font-bold tracking-widest text-white/50 uppercase block mb-1">Subconscious Goal</span>
                            <h5 className="text-2xl font-bold text-white tracking-tight leading-none">What Your Brain Wants</h5>
                        </div>
                    </div>
                    <p className="text-slate-400 text-lg leading-relaxed font-medium pl-16">
                        {info.subconsciousGoal}
                    </p>
                </div>
