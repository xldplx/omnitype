...
export default function MbtiPage() {
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(mbtiQuestions.length / questionsPerPage) - 1) {
      ...
      // Calculate results and redirect after 3s
      const calculatedResult = calculateMBTI(answers, mbtiQuestions);
      setTimeout(() => {
  ...
  // DEBUG/SAMPLE MODE: Toggle this to force INTP view for development
  const SHOW_SAMPLE_INTP = false; // Set to true to view the INTP layout
  ...
  const finalResult = SHOW_SAMPLE_INTP ? {
    type: "INTP",
    percentages: { EI: 82, SN: 65, TF: 91, JP: 74 }
  } : result;
      ...
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(mbtiQuestions.length / questionsPerPage)}</span>
          </div>
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(mbtiQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
...
function QuestionRow({ question, value, onChange, isLast }) {
  // Delicate borders and sizes for a minimal look
    ...
    <div className={`flex flex-col items-center py-12 md:py-16 border-slate-100 w-full relative ${!isLast ? 'border-b' : ''}`}>
      ...
      <div className="hidden sm:flex items-center justify-center w-full max-w-4xl mt-4">
        <div className="flex-1 flex justify-end pr-4 md:pr-8">
          <span className="text-[#6366f1] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-right leading-tight">Strongly Disagree</span>
        </div>
        ...
        <div className="flex-1 flex justify-start pl-4 md:pl-8">
          <span className="text-[#10b981] font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-left leading-tight">Strongly Agree</span>
        </div>
      </div>
...
function ResultBar({ left, right, leftCode, rightCode, leftValue, rightValue, color }) {
  ...
  const dominantCode = isLeftDominant ? leftCode : rightCode;
  const dominantValue = Math.max(leftValue, rightValue);
  
  return (
    <div className="flex flex-col gap-3">
      <div className="flex justify-between items-center text-sm md:text-base tracking-wide">
        <span className={`font-semibold ${isLeftDominant ? 'text-slate-800' : 'text-slate-400 opacity-60'}`}>{left}</span>
        <span className={`font-semibold ${!isLeftDominant ? 'text-slate-800' : 'text-slate-400 opacity-60'}`}>{right}</span>
      </div>
      
      <div className="h-3 md:h-2.5 w-full bg-slate-100 rounded-full overflow-hidden flex shadow-inner relative">
        <motion.div 
          initial={{ width: '50%' }}
        ...
        />
        <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-white z-10 opacity-50" />
      </div>
      
      <div className="flex justify-between items-center text-xs mt-1">
        <span className={`font-bold ${isLeftDominant ? color.replace('bg-', 'text-') : 'text-slate-300'}`}>
          {leftValue}%
        </span>
        <span className={`font-bold ${!isLeftDominant ? color.replace('bg-', 'text-') : 'text-slate-300'}`}>
          {rightValue}%
