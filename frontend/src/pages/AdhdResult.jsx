...
import { ChevronLeft, Heart, Target, Flame, AlertTriangle, Briefcase, MessageSquare, Users, Zap, BrainCircuit, BatteryCharging, ShieldAlert } from 'lucide-react';
    ...
    if (!stateData && type) {
      ...
      if (adhdStatuses[statusKey]) {
        stateData = {
          ...
          breakdown: { inPercent: 80, hyperPercent: 65, impulsePercent: 40 }
        };
      }
    }
    return stateData;
      ...
      >
        ...
        <Motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_40px_rgb(0,0,0,0.06)] rounded-[2.5rem] p-10 md:p-16 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 sm:gap-16">
            {[
              { label: 'Inattention', value: breakdown.inPercent, Icon: Target, delay: 0.2 },
              { label: 'Hyperactivity', value: breakdown.hyperPercent, Icon: Flame, delay: 0.4 },
              { label: 'Impulse Control', value: breakdown.impulsePercent, Icon: Zap, delay: 0.6 }
            ].map((metric) => {
              const MetricIcon = metric.Icon;
              return (
                <div key={metric.label} className="flex flex-col gap-6 group">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                        <MetricIcon className="w-5 h-5 text-slate-500" />
                      </div>
                      <span className="font-bold text-slate-800 tracking-tight text-lg">{metric.label}</span>
                    </div>
                    <span className={`font-black text-3xl tracking-tighter text-transparent bg-clip-text bg-linear-to-b ${info.color}`}>{metric.value}%</span>
                  </div>
                  ...
                  <div className="relative h-6 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-200/50 p-1">
                    <Motion.div 
                      ...
                      transition={{ duration: 1.5, ease: [0.33, 1, 0.68, 1], delay: metric.delay }}
                      className={`h-full bg-linear-to-r ${info.color} rounded-full relative overflow-hidden`}
                    >
        ...
        <Motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2.5rem] p-10 md:p-14 hover:-translate-y-2 transition-all duration-500 group">
                ...
                <Target className="w-5 h-5 text-indigo-500 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12" />
                Core Desire
                </h4>
        ...
        <Motion.div variants={itemVariants} className="bg-slate-50 border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2.5rem] p-10 md:p-16 mb-8 relative overflow-hidden group">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 relative z-10">
              {info.mbti?.map((mbtiCode) => (
                <div key={mbtiCode} className="bg-white border border-slate-200 rounded-[2rem] p-8 text-center hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] hover:-translate-y-2 transition-all duration-500">
                  <span className={`text-3xl font-black tracking-tighter leading-none text-transparent bg-clip-text bg-linear-to-b ${info.color}`}>{mbtiCode}</span>
                </div>
              ))}
            </div>
        </Motion.div>
        ...
        <Motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2.5rem] p-10 md:p-12 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500">
            ...
            <ul className="grid grid-cols-1 gap-5">
              {info.pros?.map((item) => (
                <li key={item} className="flex gap-5 items-start bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100/50 hover:bg-emerald-50 transition-colors">
          ...
          <div className="bg-white/70 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2.5rem] p-10 md:p-12 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500">
            ...
            <ul className="grid grid-cols-1 gap-5">
              {info.cons?.map((item) => (
                <li key={item} className="flex gap-5 items-start bg-rose-50/50 p-5 rounded-2xl border border-rose-100/50 hover:bg-rose-50 transition-colors">
