...
import { calculateDepressionResult, depressionQuestions } from '../utils/depressionLogic';
...
export default function DepressionTest() {
  ...
  const currentQuestions = depressionQuestions.slice(
  ...
  const handleAnswer = (questionId, value) => {
    setAnswers(prev => {
        const question = depressionQuestions.find(q => q.id === questionId);
        return {
            ...prev, 
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(depressionQuestions.length / questionsPerPage) - 1) {
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
        ...
        const calculatedResult = calculateDepressionResult(answersArray);
  ...
  if (isFinished && !result) {
    return (
      <div className="w-full min-h-screen bg-[#fafafa] flex flex-col items-center justify-center pt-24 pb-32">
        ...
        >
          <div className="w-12 h-12 border-4 border-slate-200 border-t-blue-600 rounded-full animate-spin mb-8" />
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-slate-800 tracking-tight">Mapping Emotional Currents</h2>
  ...
  if (result) {
    return <Navigate to={`/result/depression/${result.fullTitle}`} state={{ resultData: result }} replace />;
  }
    ...
    <div className="w-full min-h-screen bg-white pt-24 pb-32 relative overflow-hidden">
      <div className="w-full max-w-220 mx-auto px-4 md:px-8 mt-12 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-md sticky top-24 md:top-28 z-40 py-5 mb-16 border-b border-slate-100">
          <div className="flex justify-between items-center text-[0.65rem] md:text-xs font-bold text-slate-400 mb-4 tracking-[0.2em] uppercase">
            ...
            <span className="hidden sm:inline bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Phase {currentPage + 1} / {Math.ceil(depressionQuestions.length / questionsPerPage)}</span>
          </div>
          <div className="h-1.5 w-full bg-slate-100 overflow-hidden rounded-full">
            <motion.div 
              className="h-full bg-linear-to-r from-blue-600 to-indigo-900"
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="pt-20 pb-8 flex flex-col md:flex-row justify-center items-center w-full px-4 md:px-0">
            <div className="flex items-center gap-4">
              ...
              >
                <span className="relative z-10">{currentPage === Math.ceil(depressionQuestions.length / questionsPerPage) - 1 ? 'Analyze Data' : 'Continue'}</span>
      ...
      <div className="w-full max-w-4xl mx-auto px-6 mt-12 pb-20 text-center">
        <p className="text-[0.65rem] text-slate-400 uppercase tracking-widest leading-relaxed">
          Disclaimer: This assessment is based on standardized screening tools but does not constitute a clinical diagnosis. Please consult a medical professional for a comprehensive evaluation.
        </p>
      </div>
    </div>
  );
...
function QuestionRow({ question, value, onChange, isLast }) {
  const options = [
    { val: 7, size: 'w-18 h-18 sm:w-[4.5rem] sm:h-[4.5rem]', border: 'border-blue-600', bg: 'bg-blue-600' },
    { val: 6, size: 'w-14 h-14 sm:w-[3.5rem] sm:h-[3.5rem]', border: 'border-blue-600', bg: 'bg-blue-600' },
    { val: 5, size: 'w-10 h-10 sm:w-[2.8rem] sm:h-[2.8rem]', border: 'border-blue-600', bg: 'bg-blue-600' },
    { val: 4, size: 'w-8 h-8 sm:w-[2.2rem] sm:h-[2.2rem]', border: 'border-slate-300', bg: 'bg-slate-300' },
    { val: 3, size: 'w-10 h-10 sm:w-[2.8rem] sm:h-[2.8rem]', border: 'border-indigo-400', bg: 'bg-indigo-400' },
    { val: 2, size: 'w-14 h-14 sm:w-[3.5rem] sm:h-[3.5rem]', border: 'border-indigo-400', bg: 'bg-indigo-400' },
    { val: 1, size: 'w-18 h-18 sm:w-[4.5rem] sm:h-[4.5rem]', border: 'border-indigo-400', bg: 'bg-indigo-400' },
  ];
    ...
    <div className={`flex flex-col items-center py-12 md:py-16 border-slate-100 w-full relative ${!isLast ? 'border-b' : ''}`}>
      ...
      <div className="flex items-center justify-center w-full max-w-4xl mt-4">
        <div className="flex-1 flex justify-end pr-4 md:pr-8">
          <span className="text-blue-600 font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-right leading-tight">Very Often</span>
        </div>
        ...
        <div className="flex-1 flex justify-start pl-4 md:pl-8">
          <span className="text-indigo-400 font-semibold text-xs md:text-[0.8rem] uppercase tracking-widest text-left leading-tight">Never</span>
        </div>
      </div>
    </div>
  );
